...
const ExpensePage = () => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      setError("⚠️ Failed to fetch expenses. Please try again.");
    } finally {
      setLoading(false);
    }
  };
...
useEffect(() => {
...
}, [accessToken]); // ✅ fetch whenever the logged-in user changes
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      setError("⚠️ Failed to add expense. Please try again.");
      alert("❌ Failed to add expense");
    }
  };

  // 🔹 Function to get Spending Status
  const getStatusBadge = (amount) => {
    if (amount >= 10000)
      return <Badge bg="danger">High Spending</Badge>;
    if (amount >= 5000)
      return <Badge bg="warning" text="dark">Moderate Spending</Badge>;
    return <Badge bg="secondary">Low Spending</Badge>;
  };
  ...
  return (
    ...
    <div className="container mt-4">
      ...
      <h2 className="mb-4 text-danger">💸 Expense Transactions</h2>
        ...
        <Col md={4}>
          <Card className="shadow-sm border-danger">
            <Card.Body>
              ...
              <h4 className="text-danger fw-bold">₹{summary.total}</h4>
              <small className="text-muted">Overall spending</small>
            </Card.Body>
          </Card>
        </Col>
        <Col md={4}>
          <Card className="shadow-sm border-warning">
            <Card.Body>
              ...
              <h4 className="text-warning fw-bold">₹{summary.monthly}</h4>
              <small className="text-muted">Current month's total</small>
            </Card.Body>
          </Card>
        </Col>
        <Col md={4}>
          <Card className="shadow-sm border-secondary">
            <Card.Body>
              <Card.Title>Average Per Expense</Card.Title>
      ...
      <Button variant="danger" className="mb-3" onClick={() => setShowAddModal(true)}>
        + Add Expense
      </Button>

      {loading ? (
        <div className="text-center my-4">Loading expenses...</div>
      ) : error ? (
